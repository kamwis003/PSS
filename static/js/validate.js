...
function validate ( )
{
    ...
    if ( !USERNAME.val() || USERNAME.val().length < 5  )
    {
        ...
        USERNAME_MSG.html( "Nazwa użytkownika musi składać się z conajmniej pięciu znaków." );
        USERNAME_MSG.show();
    ...
    {
        USERNAME_MSG.html("Nazwa użytkownika musi składać się wyłącznie z małych liter");
        USERNAME_MSG.show();
        valid = false;
    }
    ...
    {
        PASSWORD_MSG.html("Hasło musi składać się z conajmniej 8 znaków");
        PASSWORD_MSG.show();
        valid = false;
    }
    ...
    {
        CONFIRM_MSG.html("Hasła nie są identyczne");
        CONFIRM_MSG.show();
        valid = false;
    }
    ...
    if ( !FNAME.val() )
    {
        FNAME_MSG.html("Pole 'imie'' nie może być puste");
        FNAME_MSG.show();
        valid = false;
    }
    ...
    if ( !LNAME.val() )
    {
        LNAME_MSG.html("Pole 'nazwisko' nie może być puste");
        LNAME_MSG.show();
        valid = false;
    }
    ...
    if ( atpos < 1 || dotpos < atpos + 2 || dotpos + 2 >= x.length ) {
        EMAIL_MSG.html("Musisz podać poprawny adres e-mail");
        EMAIL_MSG.show();
        valid = false;
    }
